...
async function getTopDonorsStreak(numberToDisplay) {
    ...
    filteredDonations.forEach(donation => {
        ...
        if (!donorStreaks[donorId]) {
            donorStreaks[donorId] = { lastDonationDate: null, currentStreak: 0, maxStreak: 0 };
        }
    ...
    topDonors.forEach(([donorId, streakInfo], index) => {
        console.log(`${index + 1}. Donor ID: ${donorId}, Streak: ${streakInfo.maxStreak} days`);
    });
...
// Example usage with a variable number to display
